    ...
    if (validate()) {
      try {
        ...
        if (result!.status == 400) {
          setExistsError("Email already exists!");
          return;
        } else {
          setExistsError("");
        }
    ...
    <div className="min-h-screen flex items-center justify-center bg-black py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
          ...
          {existsError && (
            <p className="mt-2 text-sm text-red-600">{existsError}</p>
          )}
          <div>
            <button
              type="submit"
          ...
          <div className="mt-6">
            ...
            <div>
              <Link
                href={"/login"}
                className="mt-5 group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-800 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
                Sign In
              </Link>
            </div>
